...
This example will demonstrate how to use the deployed fscio.wrap contract together with the fscio.msig contract to allow a greater than two-thirds supermajority of block producers of an FSCIO blockchain to change the owner authority of an arbitrary account. The example will use clfsc: in particular, the `clfsc multisig` command, the `clfsc set account permission` sub-command, and the `clfsc wrap exec` sub-command. However, the guide also demonstrates what to do if the `clfsc wrap exec` sub-command is not available.
